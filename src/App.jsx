                ...
                <div className="bg-white p-4 rounded-xl border-2 border-pink-200 mb-3">
                    <div className="flex items-center gap-2 mb-3">
                        <input
                            type="text"
                            readOnly
                            value={salonData?.id ? `https://app.la-vie-beauty.com.br/?salonId=${salonData.id}` : 'Carregando...'}
                            className="flex-1 bg-transparent text-xs font-mono text-gray-700 outline-none break-all"
                        />
                        <button
                            onClick={async () => {
                            ...
                            className="p-2 bg-pink-100 hover:bg-pink-200 rounded-lg transition-colors shrink-0"
                            title="Copiar link"
                        >
                            {linkCopied ? (
                    ...
                    <button
                        onClick={async () => {
                            if (!salonData?.id) {
                                alert("⚠️ Link ainda não está disponível.");
                                return;
                            }
                            
                            // Garante que o link está completo e válido
                            // Remove qualquer encoding extra e cria um link limpo
                            const salonId = salonData.id.trim();
                            
                            // Formato específico para mensagens rápidas do WhatsApp
                            // O WhatsApp precisa do link limpo, sem encoding extra
                            // e em uma linha separada para funcionar corretamente
                            const whatsappFormattedLink = `https://app.la-vie-beauty.com.br/?salonId=${salonId}`;
                            
                            try {
                                await navigator.clipboard.writeText(whatsappFormattedLink);
                                setLinkCopied(true);
                                setTimeout(() => setLinkCopied(false), 3000);
                                alert("✅ Link formatado copiado!\n\nCole este link nas suas mensagens rápidas do WhatsApp. O link será reconhecido automaticamente como clicável.");
                            } catch (err) {
                                // Fallback para navegadores antigos
                                const input = document.createElement('input');
                                input.value = whatsappFormattedLink;
                                document.body.appendChild(input);
                                input.select();
                                document.execCommand('copy');
                                document.body.removeChild(input);
                    ...
                    >
                        ...
                        <span className="whitespace-nowrap">Copiar para WhatsApp</span>
                    </button>
                </div>
