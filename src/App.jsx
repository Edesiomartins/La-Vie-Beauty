        ...
        <div className="flex flex-col h-full bg-gradient-to-b from-gray-50 to-gray-100 relative">
            ...
            <div className="bg-gradient-to-br from-pink-50 to-rose-50 p-6 rounded-3xl shadow-md border-2 border-pink-200">
                ...
                <div className="bg-white p-4 rounded-xl border-2 border-pink-200 mb-3">
                    ...
                    {/* Botão de Copiar Link para WhatsApp */}
                    <button
                        onClick={async () => {
                            if (!salonData?.id) {
                            ...
                            const link = `https://app.la-vie-beauty.com.br/?salonId=${salonId}`;
                            
                            try {
                                await navigator.clipboard.writeText(link);
                                setLinkCopied(true);
                                setTimeout(() => setLinkCopied(false), 3000);
                                alert("✅ Link copiado! Agora você pode colar nas suas mensagens rápidas do WhatsApp.");
                            } catch (err) {
                                // Fallback para navegadores antigos
                                const input = document.createElement('input');
                                input.value = link;
